...
- **Cross-cutting modules (not junk drawers; must be generic/reusable):** `database/` (Drizzle client + schema barrel + `db-errors`) and `email/` (generic Resend transport — fetch wrapper, no SDK); coming later per `architecture.md`: `instances/` (token-auth HTTP client to whitelabeled instances, phase 7 — the ONLY reader of `SHARED_INSTANCE_TOKEN`). Domain composition that *uses* a cross-cutting module stays in the domain module (push shaping in `tenants/`, reminder email content in `billing/`, setup email content in `tenants/`).
