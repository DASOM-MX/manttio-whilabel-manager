...
- **Feature folders:** `src/app/{dashboard,tenants,users,billing,blacklist}/` hold pages + per-feature components. Layout (shell/sidebar/topbar) lives in `src/app/layout/`; nav config in `src/app/core/nav/nav-items.ts` (role-aware); models in `src/app/core/models/`; pure display pipes in `src/app/pipes/`.
...
- **No inline function calls in template expressions** (interpolations or property bindings) — they re-run on every change-detection cycle and cause nasty re-rendering. Use `computed()` signals for component-level values, or **pure pipes in `src/app/pipes/`** for per-row transforms inside `@for` / `<p-table>` bodies (e.g. `tenant.status | statusSeverity`, `plan | planPrice`). Signal reads (`{{ total() }}`) and event handlers (`(click)="remove(row)"`) are fine. Pipes delegate to `data/utils.ts` helpers or `*_LABELS` records — keep the logic there, not in the pipe.
